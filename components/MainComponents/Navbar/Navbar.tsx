    ...
    <nav className="fixed top-0 w-full z-50 bg-white border-b border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)]">
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-slate-50 px-6 py-8 flex flex-col gap-6 shadow-xl">
          <Link href="/dashboard" className="text-lg font-bold text-slate-800">Dashboard</Link>
          <Link href="/team" className="text-lg font-bold text-slate-800">Team</Link>
          <Link href="/tasks" className="text-lg font-bold text-slate-800">Tasks</Link>
          <hr className="border-slate-100" />
          <div className="flex flex-col gap-4">
            <Link href="/login" className="text-center font-bold text-slate-600">Login</Link>
            <Link href="/signup" className="bg-blue-600 text-white py-4 rounded-2xl text-center font-bold">Get Started</Link>
          </div>
        </div>
      )}
    </nav>
